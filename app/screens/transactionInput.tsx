...
//       // Xử lý chuỗi để đảm bảo chỉ lấy phần JSON hợp lệ
//       responseText = responseText
//         .replace(/^```json/, "") // Loại bỏ ```json ở đầu
//         .replace(/```$/, "") // Loại bỏ ``` ở cuối
//         .replace(/```/g, "") // Loại bỏ bất kỳ ``` nào còn sót lại
//         .trim();
  
//       // Kiểm tra xem chuỗi có phải JSON hợp lệ không trước khi parse
...
//       // Kiểm tra xem JSON có đủ các trường yêu cầu không
...
//             date: categorizedData.date, // Đảm bảo date đã là DD/MM/YYYY từ Gemini
//           });
...
//             {transactionData.date || "Chọn ngày"} {/* Hiển thị DD/MM/YYYY hoặc placeholder */}
//           </Text>
//         </Pressable>
...
//                 handleInputChange("date", formattedDate); // Lưu dưới dạng DD/MM/YYYY
//               }
//             }}
//           />
//         )}
...
const TransactionInputScreen = () => {
  ...
  const handleInputChange = (name: string, value: string) => {
    if (name === "amount") {
      handleAmountChange(value); // Gọi hàm xử lý riêng cho amount
    } else {
      setTransactionData({
        ...transactionData,
        [name]: value,
